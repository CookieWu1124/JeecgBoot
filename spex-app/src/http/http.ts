    ...
    uni.request({
      ...
      success: async (res) => {
        ...
        if (res.statusCode >= 200 && res.statusCode < 300) {
          // 处理业务逻辑错误（Jeecg 同时看 code 与 success）
          if (!isSuccessResultCode(code as number) || responseData?.success === false) {
          ...
          // Jeecg 使用 result，unibest 模板使用 data
          const payload = (responseData.result !== undefined ? responseData.result : responseData.data) as T
