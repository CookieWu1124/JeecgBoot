...
<template>
  <view class="flex flex-col items-center justify-center">
    ...
    <template v-if="item.iconType === 'unocss' || item.iconType === 'iconfont'">
      <view :class="[item.icon, isBulge ? 'text-25px text-white' : 'text-23px']" />
    </template>
    <template v-if="item.iconType === 'image'">
      <image :src="getImageByIndex(index, item)" mode="scaleToFill" :class="isBulge ? 'h-25px w-25px' : 'h-23px w-23px'" />
    </template>
    <view v-if="!isBulge" class="mt-3px text-10px tracking-wide">
