...
4. **提案业务页**：UI 已齐；**除登录外多数仍为 mock**，真实联调待下一步把 `src/api/proposal.ts` 接到页面。
...
- [ ] `pnpm run dev` H5 可打开
- [ ] 真实账号登录成功（非「体验登录」）
- [ ] （下一步）发起提案打通 `POST /proposal/create` + `submit`
---

### 5.x 发起提案联调（2026-08-28）
...
4. 首页入口进入 **发起提案**，或打开对应提案页
5. 填完两步后提交；管理端「提案管理」应能查到该单（状态待审核）

> 注：发起页真实接口联调为下一步；当前 `spex-app` 提案页仍可能为 mock，以 `README.spex.md` 为准。
