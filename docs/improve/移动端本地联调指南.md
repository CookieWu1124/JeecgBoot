...
> **关联文档**：[提案改善系统-实施规划.md](./提案改善系统-实施规划.md) · [README.md](./README.md) · [**小程序接口对接清单**](./api/提案小程序-接口对接清单.md) · [**微信开发者工具 0-1 教程**](./微信开发者工具-小程序联调教程.md)  
> **创建日期**：2026-08-28
...
| 登录方式 | 工号+密码 `POST /sys/mLogin`；工号+微信授权须在小程序内（见微信开发者工具教程） |
...
| GET | `/proposal/app/me/summary` | 小程序「我的」岗位/角色/统计 |
...
3. **登录**：`/sys/mLogin` + 工号密码（须真实登录，无体验账号）。
...
4. 首页入口进入 **发起提案**，填写内容两步后提交
5. 「提案」Tab 应能看到刚提交的单据；管理端「提案管理」状态为审核中
...
## 6. 路线 D：微信开发者工具（测微信授权必用）

Chrome H5 **测不了**微信授权。从安装软件到点「微信授权登录」的逐步说明见：

**[微信开发者工具-小程序联调教程.md](./微信开发者工具-小程序联调教程.md)**

最短路径：`spex-app` 执行 `pnpm run dev:mp` → 用开发者工具打开 `dist/dev/mp-weixin` → 勾选不校验合法域名 → 先测工号+密码，再测工号+微信授权。
