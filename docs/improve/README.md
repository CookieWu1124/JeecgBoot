...
| 其它业务复用同一套微信登录 | `微信小程序登录-其他模块接入.md` | 小程序清单 §3 A1a/A1b |
...
8. **其他模块接入微信登录** → `微信小程序登录-其他模块接入.md`：同一 AppID 只改前端；新 AppID 才扩配置
9. **小程序原型** → `prototype/improveSys.html`：23 页页面结构与字段
10. **管理端原型** → `prototype/index.html`：提案管理 + 提案配置（忽略其他菜单）
11. **人员导入** → `data/README.md` + `data/员工列表20260822.xls`：组织树与用户字段映射
12. **建表脚本** → `sql/proposal_init.sql`：提案业务表 + `sys_role` 初始化
