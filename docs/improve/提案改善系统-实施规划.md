...
| 状态码（建议） | 显示名 | 说明 |
|---------------|--------|------|
| `PENDING_REVIEW` | 审核中 | 申请已提交，委员审核中 |
| `PENDING_APPROVAL` | 待批准 | 委员意见汇总，等批准人 |
| `APPROVED` | 已批准 | 申请段通过；**不要与待指派合并**（阶段 2/3/4 待定） |
| `REJECTED_FINAL` | 不批准 | 申请阶段终态，不可退回 |
| `PENDING_ASSIGN` | 待指派 | 阶段 2 待定；不是申请批准后的即时状态 |
...
| 申请审核 | 在任委员会成员**并行独立**审核（人数 = 名册快照）；申请段**不可撤回** |
...
```mermaid
flowchart TD
    A[进入审核中] --> B[按名册快照创建 N 条待办<br/>proposal_committee_review]
    B --> C{委员i提交?}
    C -->|否| D[状态保持审核中<br/>进度 N_done/N]
    D --> C
...
**Step 2 — 确认提交**：摘要展示 + `提交后进入委员会审核，不可撤回`
...
| 移动端 | uni-app（unibest） | 本仓库 `spex-app/` |
| 数据库 | MySQL | 8.0 |
...
| status | varchar(32) | PENDING_REVIEW / PENDING_APPROVAL / APPROVED / REJECTED_FINAL |
...
└── jeecg-module-spex-inside/
    ...
    └── src/main/java/org/jeecg/modules/proposal/
        ...
        │   ├── ProposalController.java    # 创建/列表/详情/委员审/批准
...
```
spex-app/  （unibest，提案改善移动端主工程）
└── src/
    ├── api/
    │   ├── login.ts
    │   └── proposal.ts          # 业务 API（联调中）
    ├── pages/
    │   ├── index/               # 首页
    │   ├── todo/                # 我的待办
    │   ├── proposal/
...
| 方法 | 路径 | 说明 |
|------|------|------|
| POST | `/proposal/create` | 发起提案（一次提交，进入审核中） |
...
| 3 | 是否需要草稿功能？ | **不需要**；申请段已取消暂存，发起即审核中 |
